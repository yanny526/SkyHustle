# handlers.py
# Definitive, Unabridged Version for System 9.3 (Alliance Interface)
...
def send_build_menu(bot, user_id):
    ...
    if build_item_id := player_data.get('build_queue_item_id'):
        ...
        markup = InlineKeyboardMarkup().add(InlineKeyboardButton("⬅️ Back to Base", callback_data='alliance_back')) # Using a generic back
    else:
        ...
        markup.add(InlineKeyboardButton("⬅️ Back to Base", callback_data='alliance_back'))
...
def handle_upgrade_request(bot, scheduler, user_id, building_key, message): pass # For brevity
...
def send_alliance_menu(bot, user_id):
    _, player_data = google_sheets.find_player_row(user_id)
    if not player_data: return
    alliance_id = player_data.get('alliance_id')
    if not alliance_id:
        send_no_alliance_menu(bot, user_id)
    else:
        bot.send_message(user_id, f"You are in alliance [ID: {alliance_id}]. Dashboard coming next!")

def send_no_alliance_menu(bot, user_id):
...
def register_handlers(bot, scheduler):
    ...
    @bot.message_handler(commands=['attack', 'start'])
    def command_handler(message: Message):
        # Logic to route to different command handlers
    ...
    def handle_callback_query(call):
        ...
        # A more robust parser for the callback data
        ...
        if command == 'alliance':
            ...
            # Add other alliance sub-commands here in the future
            elif key == 'back': # A generic back for alliance menus
                 _, pd = google_sheets.find_player_row(user_id); send_alliance_menu(bot, user_id) if pd else None
        ...
        # Note: 'back_to_base' is now handled by the 'alliance_back' or similar generic backs.
    ...
    def handle_menu_buttons(bot, message: Message):
        if message.text == constants.MENU_ALLIANCE:
            send_alliance_menu(bot, message.from_user.id)
