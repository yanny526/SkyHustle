# handlers.py
# Final Hotfix: Corrects a SyntaxError in handle_upgrade_request.
...
def battle_resolution_job(bot, scheduler, attacker_id, defender_id):
    ...
    attacker_updates = {
        ...
        'return_queue_army_data':json.dumps(attacker_survivors), 'return_queue_finish_time':return_time.isoformat(),
        **{u['id']:0 for u in constants.UNIT_DATA.values()}
    }
    if attacker_wins:
...
def send_attack_confirmation_menu(bot, user_id, attacker_data, defender_data):
    ...
    for key, unit in constants.UNIT_DATA.items():
        if count := int(attacker_data.get(unit['id'], 0)):
            army_comp += f"{count}x {unit['name']} {unit['emoji']}, "; total_units += count
