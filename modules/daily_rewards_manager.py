...
class DailyRewardsManager:
    def __init__(self):
        self.data_file = "data/daily_rewards.json"
    ...
    def _load_data(self):
        ...
        try:
            with open(self.data_file, 'r') as f:
                data = json.load(f)
                self.last_claim = data.get('last_claim', {})
                self.streaks = data.get('streaks', {})
                self.claimed = {k: set(v) for k, v in data.get('claimed', {}).items()}
    ...
    def _save_data(self):
        ...
        data = {
            ...
            'claimed': {k: list(v) for k, v in self.claimed.items()}
        }
