...
async def receive_name(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    try:
        ...
        print(f"✅ [receive_name] Name received: {name}")
        ...
        print(f"🔍 [receive_name] get_user_by_name -> {existing}, row: {row}")
        ...
        print(f"🧾 [receive_name] add_new_user returned: {success}")

        if not success:
            await update.message.reply_text("⚠️ Failed to create commander. Please try again.")
            return ConversationHandler.END

        context.user_data["game_name"] = name
        res = get_user_resources(name)
        print(f"📦 [receive_name] get_user_resources returned: {res}")
