...
def alliance(update: Update, context: CallbackContext):
    ...
    if not context.args or len(context.args) < 1:
        show_alliance_status(update, context, player)
        return
...
def show_alliance_status(update: Update, context: CallbackContext, player):
...
def war(update: Update, context: CallbackContext):
    ...
    # Check if subcommand was provided
    if not context.args or len(context.args) < 1:
        show_war_status(update, context, player, alliance_data)
        return
...
def show_war_status(update: Update, context: CallbackContext, player, alliance_data):
