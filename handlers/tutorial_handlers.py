...
async def start_tutorial(update: Update, context: CallbackContext):
    ...
    if update.callback_query:
        ...
        await query.edit_message_text(TUTORIAL_STEPS[0]["message"])
    else:
        # If called directly from a command
        user = update.effective_user
        user_id = user.id
        await update.message.reply_text(TUTORIAL_STEPS[0]["message"])
...
async def process_tutorial_step(update: Update, context: CallbackContext):
    ...
    if not next_step_id:
        ...
        if next_step:
            ...
            await update.message.reply_text(next_step["message"])
