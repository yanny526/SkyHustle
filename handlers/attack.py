...
PEND_HEADER = ["job_name","uid","defender_id","defender_name","composition",
...
async def attack(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # default: all troops if not specifying main attack or units
    ...
    if scout_count:
        lines.append(f"• 🔎 Scouts arriving in 5m")
    if not scout_only:
        lines.append(f"• 🏹 Attack arriving in 30m")
    if parts:
        lines.append("\n• " + "  ".join(parts))
